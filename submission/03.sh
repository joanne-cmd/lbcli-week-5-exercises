# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

# Extract the multisig redeem script from scriptSig.
redeem_script=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].scriptSig.asm' | awk '{print $NF}')

# Build P2WSH witness redeem script
witness_redeem_script="0020$(echo -n "$redeem_script" | xxd -r -p | sha256sum | awk '{print $1}')"

# Return the P2SH wrapper address for that witness redeem script.
bitcoin-cli -regtest decodescript "$witness_redeem_script" | jq -r '.p2sh'
